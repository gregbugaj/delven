...
class ExplicitASTNodeVisitor extends ASTVisitor<void> {
  ...
  visitModule(node: Node.Module): void {
    this.write("// Generated by Delven transpiler ver 0.0.2", false);
    this.writeNewLine();

    for (let i = 0; i < node.body.length; ++i) {
      const stm = node.body[i];
  ...
  visitStatement(statement: Node.Declaration | Node.Statement) {
    // block statetements are handled differently
    let prolog = true;
    ...
    switch (statement.type) {
      ...
      case Syntax.ExportNamedDeclaration: {
        this.visitExportNamedDeclaration(
          statement as Node.ExportNamedDeclaration
        );
        break;
      }
      case Syntax.ExportDefaultDeclaration: {
        this.visitExportDefaultDeclaration(
          statement as Node.ExportDefaultDeclaration
        );
        break;
      }
    ...
    switch (expression.type) {
      ...
      case Syntax.ArrowFunctionExpression: {
        this.visitArrowFunctionExpression(
          expression as Node.ArrowFunctionExpression
        );
        break;
      }
      ...
      case Syntax.OptionalCallExpression: {
        this.visitOptionalCallExpression(
          expression as Node.OptionalCallExpression
        );
        break;
      }
      case Syntax.MemberExpression: {
        this.visitMemberExpression(
          expression as
      ...
      case Syntax.ConditionalExpression: {
        this.visitConditionalExpression(
          expression as Node.ConditionalExpression
        );
        break;
      }
      ...
      case Syntax.OptionalMemberExpression: {
        this.visitOptionalMemberExpression(
          expression as Node.OptionalMemberExpression
        );
        break;
      }
      ...
      case Syntax.TaggedTemplateExpression: {
        this.visitTaggedTemplateExpression(
          expression as Node.TaggedTemplateExpression
        );
        break;
      }
      default:
    ...
    switch (statement.declaration.type) {
      case Syntax.FunctionDeclaration:
        this.visitFunctionDeclaration(
          statement.declaration as Node.FunctionDeclaration
        );
        break;
      default:
  ...
  visitIfStatementConsequent(consequent: Node.Statement) {
    if (consequent.type === Syntax.ExpressionStatement) {
      this.visitExpressionStatement(consequent as Node.ExpressionStatement);
    } else {
      this.visitStatement(consequent);
    }
  }

  /**
   * Non blocked if statetement are not properly rendering
   *
   * Example:
   * ```
   *
  ...
  visitUnaryExpression(expression: Node.UnaryExpression): void {
    ...
    this.writeConditional(
      expression.operator === "typeof" || expression.operator === "void",
      " ",
      false
    );
    this.visitExpression(expression.argument);
  ...
  visitMemberExpression(
    ...
    if (
      expression instanceof Node.StaticMemberExpression ||
      expression instanceof Node.OptionalMemberExpression
    ) {
