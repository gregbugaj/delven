...
describe("Enumerable SkipWhile", () => {
    ...
    test("SkipWhile-predicate-true", async () => {
        const enumerable = Enumerable.of([1, 2, 1, 2, 3, 2, 3])
    ...
    test("SkipWhile-predicate-false", async () => {
        ...
        const enumerable = Enumerable.of(data)
    ...
    test("SkipWhile complex predicate is true", async () => {
        ...
        let enumerable = [5000, 2500, 9000, 8000, 6500, 4000, 1500, 5500].asEnumerable()
        let results = await enumerable.SkipWhile((amount, index) => amount > index * 1000).toArray()
        let expectedResult = [4000, 1500, 5500]
        expect(results).toEqual(expectedResult)
    })

    test("SkipWhile-value less than iterable", async () => {
        let enumerable = [1, 2, 3, 4, 5, 6, 7, 8].asEnumerable()
        let results = []
        for await (const val of enumerable.SkipWhile(x => x <= 4)) {
            results.push(val)
        }
        let expectedResult = [5, 6, 7, 8]
        expect(results).toEqual(expectedResult)
